...
# fib_main.asm
#
# Main program to compute Fibonacci numbers using both iterative and recursive
#
# In order for this to run the functions 'iterative_fibinnoci' and 
# 'recursive_fibinnoci' must be implemented and linked in another file.
#
# Memory Organization:
#   0x0000-0x1fff : text
#   0x2000-0x3fff : data
...
main:
	...
	# segment.
FIB_LOOP:
	# Set up argument for call to iterative fibonocci
	mv a0, s0
	jal iterative_fibonacci
	# Save the result into s2
	mv s2, a0
	# Set up argument for call to recursive fibonocci
	mv a0, s0	
	jal recursive_fibonacci
...
final_add:
	...
	# Done here!
END:
	ebreak # Vivado should stop here
	# System call Exit (93) with code 0 (for RARS)
	li a0,0
	li a7, EXIT_CODE
	ecall
	ebreak
