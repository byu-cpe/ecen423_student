...
//////////////////////////////////////////////////////////////////////////////////
//
//  Filename: tb_riscv_final.sv
//
//  Author: Mike Wirthlin
//  
//////////////////////////////////////////////////////////////////////////////////

module tb_riscv_final #(
    ...
    riscv_final #(.INITIAL_PC(TEXT_ADDRESS))
    ...
    riscv_final_model #(.TEXT_MEM(TEXT_MEM), .DATA_MEM(DATA_MEM), .TEXT_ADDRESS(TEXT_ADDRESS), 
    ...
    initial begin
        ...
        if (PRINT_MEMORY) begin
            $display("Final Memory Contents:");
            // Iterate over 8 sets of 4 words
            for(int i = 0; i < 8; i++) begin
                $display("Mem[0x%8h] 0x%8h 0x%8h 0x%8h 0x%8h", DATA_ADDRESS + i*4*4, 
                    riscv_model.data_memory.memory_contents[i*4], 
                    riscv_model.data_memory.memory_contents[i*4+1],
                    riscv_model.data_memory.memory_contents[i*4+2],
                    riscv_model.data_memory.memory_contents[i*4+3]
                    );
            end
        end
        $finish;
...
module riscv_final_model #(
    ...
    logic [31:0] if_PC, id_PC, ex_PC, mem_PC, wb_PC, mem_jump_target, ex_jump_target;
    ...
    logic insert_ex_bubble, load_use, load_use_condition;
    logic mem_branch_taken, mem_PCSrc, wb_PCSrc; // wb_branch_taken;
    ...
    assign iMemRead = ~(load_use && ~mem_PCSrc); // Stall on load-use hazard but not if there is a branch/jump in MEM stage (since that bubble will also cause a stall)
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (insert_ex_bubble)
                ...
                ex_PC <= id_PC;
            if (mem_PCSrc) // insert buggle in MEM
                mem_PC <= 'x;
            else
                mem_PC <= ex_PC;
            wb_PC <= mem_PC;
            ...
            if (ex_inst.is_jump())
                mem_alu_result <= ex_PC + 4; // for jal and jalr, the value to write back is PC+4 (address of next instruction)
            else
            ...
            if (iMemRead) begin
                if (mem_PCSrc) begin
                    ...
                    else if (mem_inst.is_jump())
                        if_PC <= mem_jump_target;
                end else begin
                    if_PC <= if_PC + 4;
                end
                id_PC <= if_PC;
    ...
    // EX stage
    assign insert_ex_bubble = load_use || mem_PCSrc || wb_PCSrc;
    always @(posedge clk or posedge rst) begin
        if (initialized === 1'b1) begin // initialization initializes static members
            ...
            load_use = forwarding.load_use_condition(id_inst, ex_inst) && !mem_PCSrc;
            ex_readData1 = ex_inst.is_lui() ? 32'h0 : regfile.read_rs1_i(ex_inst.u);
            ...
            // Jump target: for JAL, target is PC + imm; for JALR, target is rs1 + imm (ignoring LSB since instructions are aligned)
            ex_jump_target = (ex_inst.is_jal()) ? 
                    (ex_PC + ex_inst.imm_j()) :                          // jal
                    ((ex_operand1 + ex_inst.imm_i()) & 32'hFFFFFFFE);    // jalr
        end
    end
    // MEM stage
    always @(posedge clk) begin
        ...
        if (!rst && initialized === 1'b1) begin
            ...
            mem_branch_taken = mem_inst.is_branch() && mem_inst.branch_taken(mem_alu_result);
            MemRead_i = mem_riscv_ctrl.MemRead;
            MemWrite_i = mem_riscv_ctrl.MemWrite;
            mem_PCSrc = mem_branch_taken || mem_inst.is_jump();
        end
    end
    // WB stage
    always @(posedge clk or posedge rst) begin
    ...
    always@(negedge clk) begin
        if (initialized != 0) begin
            ...
            if (! (^id_PC === 1'bX)) begin
                $write("I=0x%8h [%s]",id_inst.u, id_inst.ext_inst_str());
                if (!iMemRead)
                    $write(" Load Use Stall");
                if (insert_ex_bubble)
                    $write(" (insert bubble)");
            ...
            if (! (^ex_PC === 1'bX)) begin
                $write("I=0x%8h [%s] ",ex_inst.u,ex_inst.ext_inst_str());
                if (mem_PCSrc)
            ...
            if (! (^mem_PC === 1'bX)) begin
                $write("I=0x%8h [%s] ",mem_inst.u, mem_inst.ext_inst_str());
            ...
            if (mem_inst.is_branch()) begin // Only print a branch taken/not taken message for branch instructions
                if (mem_inst.branch_taken(mem_alu_result))
                    $write("Branch taken to PC=0x%8h ", mem_PC + mem_inst.imm_b());
                else
                    $write("Branch not taken ");
            end
            if (mem_inst.is_jump()) begin
                if (mem_inst.is_jump())
                    $write("Jump to PC=0x%8h ", mem_jump_target);
            end
            $display();

            $write("  WB: PC=0x%8h ",wb_PC);
            if (! (^wb_PC === 1'bX)) begin
                $write("I=0x%8h [%s] ",wb_inst.u,wb_inst.ext_inst_str());
